...
중간 텐서인 tmp는 임시 변수로 활용되어 절대 전역 메모리에 쓰이지 않습니다. 이는 메모리 트래픽을 반토막낼 뿐만 아니라(x를 한 번 읽고, y를 한 번 쓸 뿐입니다) 두 번째 커널을 실행하며 발생할 오버헤드 역시 제거합니다.
...
\]
{{</rawhtml>}}
이 타일링 알고리즘은 매 반복마다 바깥쪽 합(부분곱 하나)에서 한 개의 항을 계산합니다. 블록의 스레드들은 각각 하나의 결과인 `C_tile`을 A와 B의 타일을 불러오고, 칩 안에서 그들의 곱을 계산한 후에 그 결과를 누적하는 것을 k차원에 반복해 얻어냅니다. 이는 **불러오기(Load)**, **동기화(Synchronize)**, 그리고 **연산(Compute)** 이라는 3단계의 패턴으로 이루어집니다.
